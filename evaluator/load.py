...
gsa_dir = os.path.join(script_dir, "..", "Grounded-Segment-Anything")  # Go up one level and into the correct folder
dino_path = os.path.join(gsa_dir, "GroundingDINO")
sam_path = os.path.join(gsa_dir, "segment_anything")
if dino_path not in sys.path:
    sys.path.append(dino_path)  # Add GroundingDINO to Python path
...
def reassign_labels_1(embed_store, image, boxes, masks, device,  big_object, small_object, phrases,  clip_t, full_picture):
...
def reassign_labels_2(embed_store, image, boxes, masks, device,  big_object, small_object, phrases,  clip_t, full_picture):
...
def reassign_labels_3(embed_store, image, boxes, masks, device,  big_object, small_object, phrases,  clip_t, full_picture):
    ...
    for index, (box, mask) in enumerate(sorted(zip(boxes, masks), key = lambda x: np.sum(x[1]))):
        ...
        invert_mask = 1 - mask  # Works if best_mak is binary (0/1)
...
def reassign_labels_4(embed_store, image, boxes, masks, device,  big_object, small_object, phrases,  clip_t, full_picture):
    ...
    #examine whether CLIP on large scale has some influence
...
def get_grounding_output(model, image, caption, box_threshold, text_threshold, iou_threshold, small_object, big_object, H, W, with_logits=True, device="cpu"):
    ...
    for i, caption in enumerate(c_orig):
        ...
        logits.append(outputs["pred_logits"].cpu().sigmoid()[0])  # (nq, 256)
        boxes.append(outputs["pred_boxes"].cpu()[0])  # (nq, 4)
    ...
    #print('BEFORE NMS', boxes_filt.shape)
    # Apply NMS
    #print(boxes_filt)
    ...
    # get phrase
    #print('After NMS', boxes_filt.shape)
    ...
    #print('After logit zip', len(boxes), torch.cat(boxes, dim = 0).shape)
    if len(boxes) == 0:
        return [], []
...
def show_box(box, ax, label):
    ...
    # Add the label text with neon pink color and larger font
    ax.text(
        x0, y0, label,
        color=(1.0, 0.431, 0.78),  # Neon pink
        fontsize=18,              # Increase as needed
        weight='bold',
...
def main_gsam(model, predictor, args, clip_t, imgs, prompts, names = None):
    ...
    for idx, (image_pil, text_prompt) in enumerate(zip(imgs, prompts)):
        ...
        #print(text_prompt, 'Number of bxoes', boxes_filt.shape[0])
        ...
        total_areas.append( label_func(embed_store, image, boxes_filt.numpy().astype(np.uint16), masks.to('cpu').squeeze(1).numpy().astype(np.uint8), device,  big_object, small_object, pred_phrases, clip_t, full_picture))
