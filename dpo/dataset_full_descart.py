...
def get_ds(path, train_save_path, val_save_path):
    ...
    for group in groups:
        ...
        if not(is_positive_here and len(rejected_prompts[group[0]]) > 0):
    ...
    for group in groups:
        ...
        for i, row in group[1].iterrows():
            ...
            if row['reward'] == 1.5:
                for rejected_text in rejected_prompts[group[0]]:

                    triplet = {'rejected':rejected_text, 'chosen':row['output'], 'input':row['input'], 'objects':row['objects'], 'categories':row['categories']}
                
                    if row['categories'] not in categories_val:
                        val_ds.append(triplet)
                        add_to_val = True

                    else:
                        train_ds.append(triplet)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_save_path", type=str, default="/scratch/leuven/370/vsc37045/resource/dpo/dataset/train_full.json")
    parser.add_argument("--val_save_path", type=str, default="/scratch/leuven/370/vsc37045/resource/dpo/dataset/val_full.json")
